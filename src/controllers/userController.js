...
const userLogin = async (req, res) => {
  ...
  if (!user) {
    return res.send("email-id or password is invalid");
  }
  const token = jwt.sign({ userId: user._id }, "my-ultra-strong-password");
...
const userUpdate = async (req, res) => {
  ...
  if (!user) {
    return res.send({ status: true, msg: "User is invalid" });
  }
